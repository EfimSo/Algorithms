	...
	def minCut(self, source, sink):
		...
		for i in range(self.ROW):
			for j in range(self.COL):
				...
				self.org_graph[i][j] > 0 and visited[i] and (not visited[j]):
					print (str(i+1) + " - " + str(j+1))
		print(self.org_graph)
		print(self.graph)
		for i in range(len(visited)):
			print(f"{i+1}: {visited[i]}")
...
graph = [[0, 16, 13, 0, 0, 0],
		...
		[0, 0, 0, 7, 0, 4],
		[0, 0, 0, 0, 0, 0]]
# Find the max-flow graph and residual graph with highlighted reachable nodes in this folder
g = Graph(graph)
